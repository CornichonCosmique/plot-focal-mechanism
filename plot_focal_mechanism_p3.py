...
class drawTheseBeachballs(tk.Frame):
	...
	def ouvrir(self,fichier) :
		...
		fic = ""
		repfic = filedialog.askopenfilename(title="Select CSV file", initialdir=rep,initialfile=fic, filetypes = [("All", "*"),("CSV","*.csv")]) 
		if len(repfic) > 0:
	...
	def select_rep(self,repertoire) :
		rep = os.getcwd()
		rep = filedialog.askdirectory(title="Select output directory", initialdir=rep) 
		if len(rep) > 0:
			rep = rep
			os.chdir(rep)
		repertoire.set(rep)
	...
	def erreur(self,texte) :
		messagebox.showerror("Error", texte)
